...
class waterMarking {
    ...
    addWatermark = async (imageUrl) => {
        try {
            let mainImage = await jimp.read(imageUrl);
            let watermark = await jimp.read(this.options.waterMarkImage);

            let [newHeight, newWidth] = this.getDimensions(mainImage.getHeight(), mainImage.getWidth(), mainImage.getHeight(), mainImage.getWidth(), this.options.ratio);
            watermark.resize(newWidth, newHeight);
            let positionX = (mainImage.getWidth() - newWidth) / 2;     //Centre aligned
            let positionY = (mainImage.getHeight() - newHeight) / 2;   //Centre aligned
            ...
            let dataFetched = mainImage.quality(100).getBufferAsync(jimp.MIME_JPEG);
